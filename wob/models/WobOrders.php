...
class WobOrders extends WobActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('adress', 'length', 'max'=>1024),
			array('hash', 'length', 'max'=>128),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, id_shop, id_status, id_order_shop, id_currency_1, id_currency_2, amount_1, amount_2, amount_paid, course, adress, hash, count_confirm, is_active, create_date, mod_date', 'safe', 'on'=>'search'),
		);
	}
	...
	public function getListByShop($id_shop)
	{
		...
		/*$criteria->with = array('shop' => array(
			'select'=>false,
			'joinType'=>'INNER JOIN',
			'on'=>'shop.id=:id_shop',
		));*/
	...
	public function setPayCurrency($id_currency)
	{
		...
		$address = Wob::wallet($currency_1->code)->getNewAddress($shop->name);
		if ($address===false) {
	...
	public function setStatusFinish()
	{
		...
		try {
			if (WobShopsWallet::up($this->id_shop, $this->amount_paid, $this->id_currency_1)) {
