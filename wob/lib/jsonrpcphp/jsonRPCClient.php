...
class jsonRPCClient {
	 ...
	 * @param boolean $debug
	 */
	public function __construct($url, $proxy = NULL, $debug = NULL) {
// server URL
		$this->url = $url;
// proxy
		is_null($proxy) ? $this->proxy = '' : $this->proxy = $proxy;
// debug state
		is_null($debug) ? $this->debug = false : $this->debug = true;
// message id
		$this->id = 1;
	}
...
// performs the HTTP POST
		...
		if ( $fp = fopen($this->url, 'r', false, $context) ) {
			...
			throw new Exception('Unable to connect to '.$this->url.': '.$php);
		}
